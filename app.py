...
def index():
    ...
    stats = {
        ...
        'rev_table' : df2.groupby(['Category','App'])[['Reviews','Rating']].max().sort_values(by = 'Reviews', ascending = False).reset_index().head(10).to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
}
    ...
    df_new = df2.groupby(['Price']).agg(
    {'Price' : 'count'}
    ).rename({'Price':'Total Downloads'}, axis = 1
    ).sort_values(by = 'Price', ascending = True).head()
    df_new.plot.line(ylabel = 'Number of Users')
    plt.savefig('df_new.png', bbox_inches = "tight")
